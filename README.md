...
When you have a array in a `struct` you can make them more efficient. To do so we can apply one of the optimisations to the frame. Currently we have two optimisations:
...
```
**Important notice!!** When using the string optimisation you have to null-terminate the string. Reading after the null-terminator is Undefined behaviour. 
